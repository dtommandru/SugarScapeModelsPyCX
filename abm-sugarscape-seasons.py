...
def initialize():
    ...
    for ag in agents:
        ...
        ag.metabolism = random.randint(10, 30)
...
def observe():
    ...
    title("Sugar Availability during"+" "+str(season))
    axis('image')
    xlim([0, 49])
    ylim([0, 49])
    xticks([])
...
def update():
    ...
    for a in agents[:]:
        if (a.sugar < 0):
            ...
            new_agent = agent()
            new_agent.x,new_agent.y  = current_position
            new_agent.metabolism = random.randint(10, 20)
            new_agent.sugar = random.randint(50,100)
            new_agent.vision = random.randint(10,20)
            agents.append(new_agent)
            dcount = dcount+1
    ...
    if t%50 == 0:
        if season ==  "Winter":
            a,b = np.arange(x_lim),np.arange(y_lim)
            positions = (product(a, b))
            del a,b

            for position in positions:
                    sugar[sugar_positions[position]].max_capacity = random.randint(2,10)
                    sugar[sugar_positions[position]].growth_rate = random.randint(1,5)
                    if position[0] <= 24 and position[1] <= 24:
                        sugar[sugar_positions[position]].available_sugar = rd.rvs(0, 5, size=1)[0]
                        sugar[sugar_positions[position]].max_capacity = sugar[sugar_positions[position]].available_sugar+40
                    elif position[0] > 24 and position[1] <= 24:
                        rv = multivariate_normal([37, 12], [[24, 0], [0, 24]])
                        sugar[sugar_positions[position]].available_sugar =int(np.round(rv.pdf([position[0],position[1]])*100000,0))
                        sugar[sugar_positions[position]].max_capacity = sugar[sugar_positions[position]].available_sugar+40
                    elif position[0] <= 24 and position[1] > 24: 
                        rv = multivariate_normal([12, 37], [[24, 0], [0, 24]])
                        sugar[sugar_positions[position]].available_sugar =int(np.round(rv.pdf([position[0],position[1]])*100000,0))
                        sugar[sugar_positions[position]].max_capacity = sugar[sugar_positions[position]].available_sugar+40
                    else:
                        sugar[sugar_positions[position]].available_sugar = rd.rvs(0, 4, size=1)[0]
                        sugar[sugar_positions[position]].max_capacity = sugar[sugar_positions[position]].available_sugar+40
            del positions
        else:
            a,b = np.arange(x_lim),np.arange(y_lim)
            ...
            for x,y in positions:
                    sugar[sugar_positions[(x,y)]].max_capacity = random.randint(2,10)
                    sugar[sugar_positions[(x,y)]].growth_rate = random.randint(1,5)
                    if x <= 24 and y <= 24:
                        rv = multivariate_normal([37,37], [[24, 0], [0, 24]])
                        x_m , y_m = mirrorImage(1,1,-50,x,y)
                        sugar[sugar_positions[(x,y)]].available_sugar =int(np.round(rv.pdf([(abs(x_m)-1) , (abs(y_m)-1)])*100000,0))
                        sugar[sugar_positions[(x,y)]].max_capacity = sugar[sugar_positions[(x,y)]].available_sugar+40

                    elif x > 24 and y <= 24:
                        sugar[sugar_positions[(x,y)]].available_sugar = rd.rvs(0, 4, size=1)[0]
                        sugar[sugar_positions[(x,y)]].max_capacity = sugar[sugar_positions[(x,y)]].available_sugar+40
                    elif x <= 24 and y > 24: 
                        sugar[sugar_positions[(x,y)]].available_sugar = rd.rvs(0, 4, size=1)[0]
                        sugar[sugar_positions[(x,y)]].max_capacity = sugar[sugar_positions[(x,y)]].available_sugar+40
                        ...
                        rv = multivariate_normal([37, 37], [[24, 0], [0, 24]])
                        sugar[sugar_positions[(x,y)]].available_sugar = int(np.round(rv.pdf([x,y])*100000,0))
                        sugar[sugar_positions[(x,y)]].max_capacity = sugar[sugar_positions[(x,y)]].available_sugar+40
            del positions
